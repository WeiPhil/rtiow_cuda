...
void Compiler::commandline::parseArguments(std::string const &Arguments)
{
    ...
    while (!Stream.eof()) {
        ...
        /*
           ...
           GL_DEBUG_TYPE_OTHER_ARB, 1, GL_DEBUG_SEVERITY_LOW_ARB, -1,
           std::string(std::string("unknown parameter type: \"") << Param <<
           std::string("\"")).c_str());
                        }
                        if(!Stream)
                        {
                                glDebugMessageInsertARB(
                                        GL_DEBUG_SOURCE_APPLICATION_ARB,
           GL_DEBUG_TYPE_OTHER_ARB, 1, GL_DEBUG_SEVERITY_LOW_ARB, -1,
           std::string(std::string("error parsing parameter: \"") << Param <<
           std::string("\"")).c_str());
                        }
        */
    }
...
std::string Compiler::parser::operator()(commandline const &CommandLine,
...
std::string Compiler::parser::parseInclude(std::string const &Line,
...
GLuint Compiler::create(GLenum Type, std::string const &Filename, std::string const &Arguments)
{
    ...
    std::pair<files_map::iterator, bool> ResultFiles =
        this->ShaderFiles.insert(std::make_pair(Name, Filename));
    assert(ResultFiles.second);
    std::pair<names_map::iterator, bool> ResultNames =
        this->ShaderNames.insert(std::make_pair(Filename, Name));
    assert(ResultNames.second);
    std::pair<names_map::iterator, bool> ResultChecks =
        this->PendingChecks.insert(std::make_pair(Filename, Name));
...
bool Compiler::check()
{
    ...
    for (names_map::iterator ShaderIterator = PendingChecks.begin();
...
void Compiler::clear()
{
    for (names_map::iterator ShaderNameIterator = this->ShaderNames.begin();
...
bool load_binary(std::string const &Filename,
                 GLenum &Format,
...
bool save_binary(std::string const &Filename,
                 GLenum const &Format,
