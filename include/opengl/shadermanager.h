...
public:
    void addShader(std::string shaderName,
                   ...
                   Compiler &compiler)
    {
        m_shaders_map[shaderName] =
            compiler.create(shaderType, path, "--version 330 --profile core");

        m_path_map[shaderName] = path;
    }

    GLuint operator()(const std::string shaderName)
    {
    ...
    {
        assert(m_shaders_map.find(shaderName) != m_shaders_map.end());
    ...
    {
        assert(m_path_map.find(shaderName) != m_path_map.end());
